...
public class PlayerLikeController {
    ...
    @GetMapping
    public ResponseEntity<List<LikePlayerResponseDto>> getLikedPlayersByMember(@Parameter(hidden = true) @RequestHeader(value = "X-User-ID", required = false) Long memberId) {
        if (memberId == null) {
            throw new MissingHeaderException();
        }
